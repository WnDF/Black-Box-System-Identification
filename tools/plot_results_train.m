function plot_results_train(y, yhat, vaf, rmse, algo)
    %   Overlay Measured vs. Simulated Output on Training Data
    ...
    title({algo, sprintf('Training | VAF = %.2f%%, RMSE = %.2f', vaf, rmse)});
    xlabel('k')
    ylabel('$y, \hat{y}$','Interpreter','latex')
    grid on;

    save_figure([strrep(lower(algo), '-', ''), '_training_fit.png']);
end
